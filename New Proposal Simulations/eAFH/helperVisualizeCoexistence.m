classdef helperVisualizeCoexistence < handle
    ...
    properties (Hidden, Constant)
        ...
        GoodChannelColor = "#d2e0b3";
        BadChannelColor = "#e5b5a5";
        CorruptedChannelColor = "#d95319";
        AdvertisingChannelColor = "white";
        TransmittedChannelColor = "#77ac30";
        WLANChannelColor = "#99c7e5";
    end
        ...
        function obj = helperVisualizeCoexistence(simulationTime,bluetoothNodes,varargin)
            ...
            if iscell(bluetoothNodes)
                for idx = 1:numel(bluetoothNodes)
                    validateattributes(bluetoothNodes{idx}, ["bluetoothLENode","bluetoothLENodeeFH","bluetoothNode"], {'scalar'});
                end
                obj.BluetoothNodes = bluetoothNodes;
            else
                validateattributes(bluetoothNodes(1), ["bluetoothLENode","bluetoothLENodeeFH","bluetoothNode"], {'scalar'});
    ...
    methods
        function initializeVisualization(obj)
            ...
            for peripheralIdx = 1:peripheralsCount
                ...
                dimensionText = [0.035 0.678 0.3 0.3];
                dimensionBox = [0.014 0.9525 0.020 0.014];
                annotationOffsets = [0 0.027 0.022 0.028 0.025 0.025 0.025];
                ...
                channelColors = [obj.WLANChannelColor;obj.GoodChannelColor;obj.BadChannelColor;...
                    obj.TransmittedChannelColor;obj.CorruptedChannelColor];
                for idx = 1:5
                ...
                for idx = 1:numChannels(peripheralIdx)
                    ...
                    obj.pChannelFig{peripheralIdx,idx}.FaceColor = color;
                    obj.pChannelFig{peripheralIdx,idx}.Curvature = [1, 0.6];
                    plot(obj.pModelFig{peripheralIdx},lineX,lineY,":","color",[0.2 0.2 0.2]);
                    ...
                    bluetoothChannelText(idx) = text(obj.pModelFig{peripheralIdx}, bleChannelCoordinateX+1.5, ...
                        5.2, ['Channel-' num2str(idx-1)],"color", "k", "Rotation", 90, "FontSize", textFontSize,...
                    ...
                    text(obj.pModelFig{peripheralIdx}, nextChannelCoordinateX-1.5, 3.2, ...
                        [centerFreq, ' MHz'], "color", "k", "FontSize", textFontSize, "Rotation", 90, ...
                        "HorizontalAlignment", "center", "FontUnits", "normalized");
                end
                ...
                title(successRateFig{peripheralIdx}, "Success rate per channel", ...
                    ...
                    "FontUnits", "normalized", "color", [0.5 0.2 0.6]);
                obj.pRecentSuccessRateFig{peripheralIdx} = bar(successRateFig{peripheralIdx}, ...
                    (0:numChannels(peripheralIdx)-1)+1.1, zeros(1, numChannels(peripheralIdx)), ...
                    "FaceColor","#99c7e5","BarWidth", 0.3);
                obj.pOverallSuccessRateFig{peripheralIdx} = bar(successRateFig{peripheralIdx}, ...
                    (0:numChannels(peripheralIdx)-1)+0.8, zeros(1, numChannels(peripheralIdx)), ...
                    "FaceColor","#0072bd", "BarWidth", 0.3);
                legend(successRateFig{peripheralIdx}, "Recent success rate", ...
                    "Cumulative success rate","Location","None","Position", [0.707 0.348 0.08 0.056])
                % Add progress bar and display text
                annotation(obj.pCoexistenceFigs{peripheralIdx}, "rectangle",obj.ProgressBarDim, ...
                    "FaceColor","#f0f0f0");
                obj.pProgressFig{peripheralIdx} = annotation(obj.pCoexistenceFigs{peripheralIdx},"rectangle", ...
                    obj.ProgressBarDim,"FaceColor","#06bd40");
        ...
        function updateBluetoothVisualization(obj,node,rxEventData)
            ...
            if strcmpi(eventName,"PacketReceptionEnded")
                ...
                channelColor = obj.CorruptedChannelColor;
                if eventDataInfo.SuccessStatus
                    channelColor = obj.TransmittedChannelColor;
                end
                ...
                if isvalid(obj.pCoexistenceFigs{peripheralIdx})
                    ...
                    updateChannelMap(obj,channelMap,peripheralIdx);
                    performanceStatistics(obj,node,obj.pPeripheralNodes{peripheralIdx},peripheralIdx,eventDataInfo.CurrentTime);
        ...
        function classificationStats= classificationStatistics(obj,centralNode,peripheralNode)
            ...
            % classification
            performanceStatistics(obj,centralNode,peripheralNode,peripheralIndex,obj.SimulationTime);
    ...
    methods (Access = private)
        ...
        function addAnnotationBox(obj,dimensionBox,boxColor,modelIdx)
            ...
            annotation(obj.pCoexistenceFigs{modelIdx},'rectangle',dimensionBox,...
                'FaceColor',boxColor,'Units','normalized');
        end
        function displayTransmissionStatus(obj,currentTime,channelNumber,channelColor,peripheralIndex)
            ...
            obj.pChannelFig{peripheralIndex, channelNumber}.FaceColor = ...
                channelColor;
            pause(0.05) % To visualize the active Bluetooth channel
            obj.pChannelFig{peripheralIndex, channelNumber}.FaceColor = ...
                obj.GoodChannelColor;
        end
        ...
        function updateChannelMap(obj,channelMap,peripheralIndex)
            ...
            for idx = 1:numel(channelMap)
                if channelMap(idx) == 1
                    obj.pChannelFig{peripheralIndex, idx}.FaceColor = obj.GoodChannelColor;
                else
                    obj.pChannelFig{peripheralIndex, idx}.FaceColor = obj.BadChannelColor;
                end
            end
        end
            ...
            for wlanIndex = 1:numel(obj.WLANNodes)
                ...
                for peripheralIdx = 1:obj.PeripheralCount
                    ...
                    wlanChannel = rectangle(obj.pModelFig{peripheralIdx}, "Position", ...
                        ...
                        wlanChannelHeight], "FaceColor", obj.WLANChannelColor, "Clipping", "off");
                    ...
                    text(obj.pModelFig{peripheralIdx}, wlanTextPos, wlanChanPos(2)+0.18, ...
                        ...
                        num2str(signalPeriodicity) ')'],"FontUnits", "normalized", "color", "black");
        ...
        function performanceStatistics(obj,centralNode,peripheralNode,peripheralIndex,currentTime)
            %performanceStatistics Calculates the packet loss ratio and
            %throughput for the time since last classification

            nodeStats = statistics(centralNode);
            % Find the stats corresponding to the Peripheral
            for peripheralIdx = 1:centralNode.NumConnections
                if obj.pIsBluetoothBREDR(peripheralIndex)
                    stats = nodeStats.Baseband.ConnectionStats(peripheralIdx);
                    if stats.PeerNodeID == peripheralNode.ID
                        break;
                    end
                else
                    stats = nodeStats.LL(peripheralIdx);
                    if strcmpi(nodeStats.App(peripheralIdx).DestinationNode,peripheralNode.Name)
                        break;
                    end
                end
            end
            % Get the total transmitted and acknowledged packets and
            % payload bytes
            if obj.pIsBluetoothBREDR(peripheralIndex)
                transmittedDataPackets = stats.TransmittedACLPackets+ ...
                    stats.TransmittedDVPackets;
                retransmittedDataPackets = stats.RetransmittedACLPackets+ ...
                    stats.DVWithRetransmittedData;
                transmittedPayloadBytes = stats.TransmittedDataBytes;
            else
                transmittedDataPackets = stats.TransmittedDataPackets;
                retransmittedDataPackets = stats.RetransmittedDataPackets;
                transmittedPayloadBytes = stats.TransmittedPayloadBytes;
            end
            % Calculate the packet loss ratio and throughput
            numClassifications = obj.NumClassifications(peripheralIndex);
            obj.PacketLossRatio(peripheralIndex,numClassifications+1) = ...
                retransmittedDataPackets/(transmittedDataPackets+retransmittedDataPackets);
            obj.Throughput(peripheralIndex,numClassifications+1) = ...
                (transmittedPayloadBytes*8*1e-3/currentTime);
        end
        function verifyConnection(obj,centralNode,peripheralNode,peripheralIndex)
            ...
            validateattributes(centralNode,["bluetoothLENode","bluetoothNode", "bluetoothLENodeeFH"],{'scalar'});
            validateattributes(peripheralNode,["bluetoothLENode","bluetoothNode","bluetoothLENodeeFH"],{'scalar'});
