		      ...
		      struct lll_conn *lll)
{
	...
	// initialize the channel classfication parameters
	lll->channel_update_counter=lll->event_counter+INITIAL_TRAINING_EVENTS;
	lll->channel_logging_events=LOGGING_EVENTS;
...
int8_t lll_afh_conn_isr(uint16_t handle, uint8_t chm[])
{
	struct ll_conn *conn;
