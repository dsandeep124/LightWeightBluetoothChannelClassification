...
void lll_conn_isr_rx(void *param)
{
	...
	if (!trx_done) {
		//lll = param;
		//lll->currfailPacketCount[lll->currentDataChannel] +=1U;
		radio_isr_set(isr_done, param);
		radio_disable();
	...
	if (crc_ok) {
		...
		if (err) {
			 ...
			 */
			//Update fail packet count			
			lll->currfailPacketCount[lll->currentDataChannel] +=1U;
			//printk("\nFailure Packet count:%llu, Data Channel:%llu\n", lll->currfailPacketCount[lll->currentDataChannel], lll->currentDataChannel);
			radio_isr_set(isr_done, param);
			radio_disable();
...
void lll_conn_isr_tx(void *param)
{
	...
	lll = param;
#if defined(CONFIG_BT_CENTRAL)
// Check if channel update is required
		//uint8_t channelList[37];		
		if(!lll->role)
		{
			//printk("CHANNEL UPDATE LOGIN IS HIT & event_counter=%u\n", lll->event_counter);
			if(lll->event_counter >= lll->channel_update_counter)
			{
				//Channel update variables
				double successRateOfChange[37]={100};
				double failureRateOfChange[37]={0};
				uint64_t currSuccCount;
				uint64_t currFailCount;
				uint64_t prevSuccCount;
				uint64_t prevFailCount;
				double rate;

				// Update the next event counter
				lll->channel_update_counter=lll->event_counter+lll->channel_logging_events;

				bool goodChannels[37], badChannels[37];
				for(uint8_t currChan=0;currChan<37;currChan++)
				{
					currSuccCount=lll->currsuccessPacketCount[currChan];
					currFailCount=lll->currfailPacketCount[currChan];
					prevSuccCount=lll->prevsuccessPacketCount[currChan];
					prevFailCount=lll->prevfailPacketCount[currChan];
					//printk(" currChan=%u, currsuccessPacketCount[currChan]=%llu\n",currChan, currSuccCount);
					//printk(" currChan=%u, currfailPacketCount[currChan]=%llu\n", currChan,currFailCount);
					
					if (currSuccCount>0)
					{
						rate = ((double)(currSuccCount-prevSuccCount)/currSuccCount)*100;
						successRateOfChange[currChan] = rate;
						}
					else
						successRateOfChange[currChan] = 0;

					if (currFailCount>0)
					{
						rate = ((double)(currFailCount-prevFailCount)/currFailCount)*100;
						failureRateOfChange[currChan] = rate;
					}
					else
						failureRateOfChange[currChan] = 0;

					lll->prevsuccessPacketCount[currChan]=currSuccCount;
					lll->prevfailPacketCount[currChan]=currFailCount;
					lll->prevtxPacketCount[currChan]=lll->currtxPacketCount[currChan];

					goodChannels[currChan]=successRateOfChange[currChan]>0;
					badChannels[currChan]=failureRateOfChange[currChan]>10;
					
					//printf("\nData Channel: %u, Success rate of change: %g\n", currChan, successRateOfChange[currChan]);
					//printf("\nData Channel: %u, Failure rate of change: %g\n", currChan, failureRateOfChange[currChan]);
				}

				uint8_t goodChannelCount=0; uint8_t new_chm[5]={0,0,0,0,0};
				for(uint8_t currChan=0; currChan<37;currChan++)
				{
					if(goodChannels[currChan])
						goodChannelCount+=1;
				}

				if(goodChannelCount==37){
					new_chm[0]=255;new_chm[1]=255;new_chm[2]=255;new_chm[3]=255;new_chm[4]=31;}
				else{
					if(goodChannelCount<2){
						printk("Good channel count is less hence reset\n");					
						new_chm[0]=255;new_chm[1]=255;new_chm[2]=255;new_chm[3]=255;new_chm[4]=31;
						}
					else{
						for(uint8_t currChan=0; currChan<37;currChan++){
							if(goodChannels[currChan] ||!badChannels[currChan])
								new_chm[currChan>>3] |= (1 << (0x07 & currChan));
						}
					}
				}
				// Update channel map
				memcpy(lll->updatedChannelMap, new_chm, sizeof(new_chm));				
				bool chan_map_updated=false;
				for (int i = 0; i < 5; i++) {						
						if (lll->data_chan_map[i] != new_chm[i])
						{			
							chan_map_updated=true;
							break;
						}
				...
				if(chan_map_updated)
				{
					printk("\nOld channel map is:%u,%u,%u,%u,%u\n", lll->data_chan_map[0],lll->data_chan_map[1],lll->data_chan_map[2],lll->data_chan_map[3],lll->data_chan_map[4]);
					printk("\nNew channel map is:%u,%u,%u,%u,%u\n", new_chm[0],new_chm[1],new_chm[2],new_chm[3],new_chm[4]);
					int8_t ret;
					ret = lll_afh_conn_isr(lll->handle, new_chm);				
					if(ret)
						printk("ISR routine failed\n");
				}


				uint64_t totalLosses=0, totaltx=0;
				for(uint8_t currChan=0;currChan<37;currChan++)
				{						
					totalLosses=totalLosses+lll->currfailPacketCount[currChan];
					totaltx=totaltx+lll->currtxPacketCount[currChan];
					}
				//printk("Total Failures=%llu\n", totalLosses);
				double plr=(double)totalLosses/(double)totaltx;
				printf("Packet Loss Ratio= %g \n", plr);
			}
		}
#endif
...
void lll_conn_pdu_tx_prep(struct lll_conn *lll, struct pdu_data **pdu_data_tx)
{
	...
	*pdu_data_tx = p;
	...
	lll->currtxPacketCount[lll->currentDataChannel]=lll->currtxPacketCount[lll->currentDataChannel]+1;
...
#error "Undefined HAL_RADIO_PDU_LEN_MAX."
	...
	if (pdu_data_rx->nesn != lll->sn) {
		...
		// Update ACK packet count
		if(!lll->role){
			lll->currsuccessPacketCount[lll->currentDataChannel] += 1U;
			//printk("\nData Channel=%u, Success Packet count:%llu\n", lll->currentDataChannel, lll->currsuccessPacketCount[lll->currentDataChannel]);
		}
