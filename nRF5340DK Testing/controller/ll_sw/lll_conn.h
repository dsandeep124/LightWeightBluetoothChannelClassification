...
#define INITIAL_TRAINING_EVENTS 2000
#define LOGGING_EVENTS 1000
...
struct lll_conn {
	...
	uint16_t channel_logging_events;
	uint64_t currtxPacketCount[37];
	uint64_t currfailPacketCount[37];
	uint64_t currsuccessPacketCount[37];
	
	uint64_t prevtxPacketCount[37];
	uint64_t prevfailPacketCount[37];
	uint64_t prevsuccessPacketCount[37];
	uint8_t currentDataChannel;
	uint8_t updatedChannelMap[5];
...
int8_t lll_afh_conn_isr(uint16_t handle, uint8_t channelList[]);